...
电脑端 httpcapture-cli
    │
    │ 二维码：电脑名称、Charles IP、端口、CA 公钥证书
    ▼
...
4. CLI 在终端显示配对二维码，也可输出二维码图片。
5. 用户打开 httpCapture APK 扫描二维码。
6. App 校验并保存 Charles 配置。
7. 如果 CA 尚未安装：Android 8–10 调起系统证书安装器；Android 11+ 将 CA 文件写入系统 Downloads 集合并打开安全设置，由用户在系统设置中确认安装。
8. 用户完成系统确认。
9. 用户选择一个或多个需要抓包的 App。
10. 用户首次启动时完成 Android VPN 授权。
11. App 将所选 App 的流量转发到 Charles。
...
2. 校验必填字段和数据长度。
3. 解析 CA 公钥证书。
4. 重新计算 CA SHA-256 指纹。
5. 校验二维码中的指纹与证书一致。
6. 检查 CA 用途、有效期及基本格式。
7. 展示电脑名称、IP、端口、证书名称、有效期和指纹。
8. 用户确认后新增或更新配置。

二维码离线携带配对数据，不要求临时 HTTP 服务、云服务或外部下载地址。
...
- 首版优先支持 Linux，后续发布 Windows、macOS 的 `amd64` 和 `arm64` 构建。
- 多平台产物应通过 CI 在对应平台执行基础测试；仅交叉编译成功不能替代目标系统验证。

选择 Go 的原因是 CLI 主要处理网络接口、X.509、公钥证书、压缩、二维码和文件操作，这些能力可以保持纯 Go。Go 官方工具链通过 `GOOS` 和 `GOARCH` 支持 Linux、Windows 和 Darwin 的多种架构，适合生成独立命令行程序。
...
| --- | --- |
| `--name` | 电脑配置名称 |
| `--host` | 手机可访问的电脑 IP 或 Host |
| `--port` | Charles 代理端口 |
| `--cert` | 手动指定 CA 公钥证书 |
| `--charles` | 手动指定 Charles 可执行文件 |
| `--output` | 输出二维码图片路径 |

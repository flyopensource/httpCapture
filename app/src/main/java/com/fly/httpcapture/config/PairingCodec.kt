...
object PairingCodec {
    ...
    fun decodeReference(value: String): PairingReference {
        ...
        val uri = runCatching { URI(raw) }.getOrElse { error("不是 HTTP Capture v2 配对二维码") }
        require(uri.scheme == "httpcapture" && uri.host == "p" && uri.query == null && uri.fragment == null) {
            "不是 HTTP Capture v2 配对二维码"
        }
        val segments = uri.path.trim('/').split('/')
        require(segments.size == 5 && segments[0] == "2") { "不支持的二维码版本" }
        val host = segments[1]
    ...
    fun decodeBundle(bytes: ByteArray, expectedSha256: String): CaptureProfile {
        ...
        require(json.getInt("v") == 2) { "不支持的配置版本" }
        ...
        val id = "${certificateSha256.take(16)}@$host:$port"
